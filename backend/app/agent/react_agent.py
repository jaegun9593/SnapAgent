...
class ReActAgent:
    ...
    def __init__(
        ...
        self.preferences = (agent.config or {}).get("preferences", {})
        ...
        try:
            while iteration < self.max_iterations:
                ...
                intent = await self.intent_classifier.classify(
                    query, tool_results, preferences=self.preferences
                )
                yield "thinking", {
    ...
    def _build_messages(
        ...
        # Preference-based instructions (supplement system prompt)
        if self.preferences:
            pref_prompt = self._build_preference_prompt(self.preferences)
            if pref_prompt:
    ...
    @staticmethod
    def _build_preference_prompt(preferences: Dict[str, Any]) -> str:
        """Build a preference instruction prompt from agent preferences."""
        purpose_map = {
        ...
        if task_purpose:
            parts.append(purpose_map.get(task_purpose, task_purpose))

        response_format = preferences.get("response_format", "")
        if response_format:
            parts.append(format_map.get(response_format, response_format))

        response_tone = preferences.get("response_tone", "")
        if response_tone:
            parts.append(tone_map.get(response_tone, response_tone))
